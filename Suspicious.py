...
import pandas as pd
from config import SUS_PATH, RAW_IMAGES_DIR, CSV_FILE_PATH
...
if not cap.isOpened():
    print("Error: Could not open video susp.mp4")
else:
    ...
    frame_total = 2000
    i = 0
    a = 295  # Start from 295 forcefully as per original script
    ...
    while cap.isOpened():
        ...
        image_path = os.path.join(RAW_IMAGES_DIR, f'susp_img_{i}.jpg')
        ...
        for r in results:
            ...
            for index, box in enumerate(bound_box):
                if conf[index] > 0.75:
                    ...
                    output_path = os.path.join(SUS_PATH, f'person_nn_{a}.jpg')
